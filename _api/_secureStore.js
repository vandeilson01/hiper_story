...
const root = process.env.SECURE_DATA_DIR || path.join(process.cwd(), 'data')
...
const fileFor = name => path.join(root, `${name}.enc.json`)
export function readSecure(name, fallback = []) {
  try {
    const raw = JSON.parse(fs.readFileSync(fileFor(name), 'utf8'))
...
export function writeSecure(name, value) {
  ...
  fs.writeFileSync(fileFor(name), JSON.stringify({ version: 1, algorithm: 'aes-256-gcm', iv: iv.toString('base64'), tag: cipher.getAuthTag().toString('base64'), data: data.toString('base64') }))
}
